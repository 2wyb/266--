...
class Corrector:
    ...
    def load_tongyinzi(self, path):
        ...
        # print(tongyinzi_dict)
        return tongyinzi_dict

    # 纠错逻辑
    def correction(self, string):
        for i in string:
            prob = self.language_model.predict(string)
            # print(prob)
            for j in self.sub_dict[i]:
                new_string = string.replace(i, j)
                string_pred = self.language_model.predict(new_string)
                # print(string_pred, new_string)
                if string_pred > prob + 7:
                    string = new_string
                    prob = string_pred
        return string
